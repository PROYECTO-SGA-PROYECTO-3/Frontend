...
interface EstudiantesSkeletonProps {
  cantidadFilas?: number
}

export function EstudiantesSkeleton({ cantidadFilas = 6 }: EstudiantesSkeletonProps) {
  return (
    <div className="flex flex-col gap-6" aria-busy="true" aria-label="Cargando información de estudiantes">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-xs">
        ...
        <div className="divide-y divide-slate-100 p-2">
          {Array.from({ length: cantidadFilas }).map((_, i) => (
            <div key={`row-skel-${i}`} className="flex items-center justify-between gap-4 p-3.5">
